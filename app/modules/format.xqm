 ...
 :)
    case element(tei:hi) return
        ...
        case element (tei:ptr) return
            ...
            <a class="ptr" href="index.html?id={collection($config:data-root)/id($node/@target/string())/ancestor::tei:TEI/@xml:id/string()}&amp;show=a&amp;view-mode=2#FN-ref_{$node/@target/string()}">{collection($config:data-root)/id($node/@target/string())/ancestor::tei:TEI//tei:titleStmt/tei:title[@level='a']/text()}</a>
            else (: Verweis auf ganzen Text :)
            <a class="ptr" href="index.html?id={$node/@target/string()}">{collection($config:data-root)/id($node/@target/string())//tei:titleStmt/tei:title[@level='a']/text()}</a>
